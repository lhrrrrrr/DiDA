...
__all__ = ['ResNet', 'resnet18', 'resnet18_pure', 'resnet18_2', 'resnet34','resnet50', 'resnet50_2','resnet50_2fc',
           'resnet50_reverse', 'resnet50_reverse2','resnet101', 'resnet50fc', 'resnet50_conv','resnet152']
...
class Base_ResNet(nn.Module):
    ...
    def __init__(self, name='resnet50', head='mlp', low_dim=512, class_num=65, feat_dim=128, pretrained=True):
        super(Base_ResNet, self).__init__()
        encoder = resnet_dict[name]
        self.encoder = encoder(pretrained=pretrained, low_dim=low_dim)
        self.fc = nn.Linear(low_dim, class_num)
        if head == 'linear':
            self.head = nn.Linear(low_dim, feat_dim)
        elif head == 'mlp':
            self.head = nn.Sequential(
                nn.Linear(low_dim, low_dim),
                nn.ReLU(inplace=True),
                nn.Linear(low_dim, feat_dim)
            )
        else:
